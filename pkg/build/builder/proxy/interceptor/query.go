...
func setQueryStringValue(values []string, key string, v reflect.Value) error {
	...
	case reflect.Map:
		...
		switch elem := v.Type().Elem(); {
		case elem.Kind() == reflect.String,
			elem.Kind() == reflect.Slice && elem.Elem().Kind() == reflect.String:
			keys = reflect.New(v.Type())
		default:
			return fmt.Errorf("value type for map %s is not supported for key %s", v.Type(), key)
		}
		if err := json.Unmarshal([]byte(last), keys.Interface()); err != nil {
